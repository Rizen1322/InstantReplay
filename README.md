...
- **Жёлтая рамка записи снимается не только через `IsBorderRequired`.** Свойство закрыто возможностью `graphicsCaptureWithoutBorder`, а согласие на неё лежит в реестре (`ConsentStore\graphicsCaptureWithoutBorder`): машинное значение на чистой Windows 11 — `Deny`, UI в «Параметрах» для него нет, и `RequestAccessAsync` молча возвращает `DeniedByUser` — при этом `IsBorderRequired = false` проходит без ошибки и читается обратно как `false`, так что по коду всё «в порядке», а рамка на экране есть. Поэтому разрешение прописывается в реестр до запроса права (`Shared/BorderlessConsent.cs`): установщик — пользовательскую часть, приложение (оно всегда elevated) — ещё и машинную.
