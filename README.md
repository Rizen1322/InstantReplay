...
- **Подача в SinkWriter — по обратной связи с диском** (`WriteDrain` в `ReplaySaver`): смотрим на реальный размер файла через собственный дескриптор и держим «в полёте» не больше 48 МБ. Писатель принимает сэмплы охотнее, чем диск их глотает, и на гигабайтном клипе разница копилась в памяти — отдали за секунду, а разгребал он полминуты, попутно вываливая гигабайт грязных страниц (отсюда фризы в игре). Фиксированный потолок «220 МБ/с», который тут стоял раньше, был угадан и мешал в обе стороны. Прогресс сохранения виден в шапке окна и на «Обзоре».
