...
/// 1. Если буфер записи уже работает — ждём у него следующий кадр монитора и
///    только на статичном экране откатываемся к последнему кадру монитора.
...
public static class ScreenshotService
{
        ...
        int monitorIndex, bool cursor, LiveFrameProvider? live = null, byte[]? into = null)
    {
        ...
        // 1) Свежий кадр всего монитора у работающего буфера.
        //
        // Обязательно В ФОНЕ, и вот почему. Внутри — ожидание ближайшего кадра
        // (до 200 мс на статичном экране), копия в видеопамяти и вычитывание
