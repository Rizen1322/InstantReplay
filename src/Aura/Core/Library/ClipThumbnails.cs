...
public static class ClipThumbnails
{
    ...
    public static async Task<ThumbResult> LoadAsync(ClipItem item)
    {
        ...
        try
        {
            // WPF-картинку можно собрать и заморозить прямо здесь: замороженная
            // BitmapImage безопасна для любого потока, ждать UI не нужно.
            var image = new BitmapImage();
            image.BeginInit();
            image.CacheOption = BitmapCacheOption.OnLoad;
            image.DecodePixelWidth = (int)ThumbWidth;
            image.StreamSource = new MemoryStream(payload.Image);
            image.EndInit();
            image.Freeze();
    ...
    private static async Task<byte[]> ToJpegAsync(IRandomAccessStream source)
    {
        ...
        var encoder = await Windows.Graphics.Imaging.BitmapEncoder.CreateAsync(
            Windows.Graphics.Imaging.BitmapEncoder.JpegEncoderId, output).AsTask().ConfigureAwait(false);
