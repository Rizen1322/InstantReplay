...
public sealed class ManualRecorder : IDisposable
{
    ...
    /// <summary>
    /// Конвейер пересобран, кадры пойдут от нового энкодера. До его первого
    /// ключевого кадра ничего не пишем: промежуточные кадры ссылаются на кадры,
    /// которых в файле нет. Пока идёт пересборка, последний кадр просто держится
    /// на экране, звук пишется без разрыва.
    /// </summary>
    public void ResumeAfterGap() => _droppingUntilKeyframe = true;
