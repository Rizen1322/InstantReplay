...
public sealed class AppSettings
{
    ...
    /// <summary>
    /// Через сколько минут без клавиатуры и мыши приостановить повтор. Ноль —
    /// не приостанавливать.
    ///
    /// Человек отошёл от компьютера — записывать нечего, а конвейер продолжает
    /// занимать около гигабайта памяти, полгигабайта видеопамяти и кодировщик.
    /// Пятнадцать минут выбраны с запасом: за столько не пройдёт ни одна катсцена
    /// и ни один экран загрузки, а вот отход за чаем укладывается.
    ///
    /// Ввод возвращается — повтор поднимается сам, ничего нажимать не нужно.
    /// </summary>
    public int PauseAfterIdleMinutes { get; set; } = 15;
