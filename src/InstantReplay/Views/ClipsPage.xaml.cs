...
///
/// Клик по карточке открывает файл в системном плеере по умолчанию. Встроенный
/// плеер здесь был, но пользы не приносил: он зависит от установленных в системе
/// декодеров ровно так же, а управление и качество — хуже любого нормального плеера.
///
...
public sealed partial class ClipsPage : Page
{
    ...
    private void MenuOpen_Click(object sender, RoutedEventArgs e) => Open(ItemOf(sender));
