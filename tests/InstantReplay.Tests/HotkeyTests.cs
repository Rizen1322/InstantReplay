...
/// <summary>
/// Кнопки мыши в сочетаниях.
///
/// Коды взяты настоящие (VK_MBUTTON/VK_XBUTTON1/VK_XBUTTON2), и это не мелочь:
/// клавиатурный хук их не выдаёт никогда, поэтому одна карта сочетаний обслуживает
/// и клавиши, и мышь — без отдельного признака «это мышь» в ключе.
/// </summary>
public class MouseHotkeyTests
{
    [Theory]
    [InlineData("Mouse3", 0x04u)]
    [InlineData("Mouse4", 0x05u)]
    [InlineData("Mouse5", 0x06u)]
    public void КнопкиМышиРазбираются(string combo, uint expected)
    {
        Assert.True(HotkeyParser.TryParse(combo, out var key));
        Assert.Equal(expected, key.vk);
        Assert.True(HotkeyParser.IsMouseButton(key.vk));
    }

    [Fact]
    public void КнопкаМышиРаботаетСМодификаторами()
    {
        Assert.True(HotkeyParser.TryParse("Alt+Shift+Mouse4", out var key));
        Assert.Equal(0x05u, key.vk);
        Assert.True(key.alt);
    ...
    [Fact]
    public void ЗаписьСочетанияСМышьюНормализуется()
    {
        Assert.Equal("Alt+Mouse5", HotkeyParser.Normalize("alt + mouse5"));
        Assert.Equal("Mouse4", HotkeyParser.Normalize("MOUSE4"));
    }

    [Fact]
    public void ИмяКнопкиЧитаетсяОбратноПоКоду()
    {
        Assert.Equal("Mouse3", HotkeyParser.KeyName(0x04));
        Assert.Equal("Mouse4", HotkeyParser.KeyName(0x05));
        Assert.Equal("Mouse5", HotkeyParser.KeyName(0x06));
    }

    [Fact]
    public void ЛеваяИПраваяКнопкиНеНазначаются()
    {
        // Их коды (0x01 и 0x02) в словаре отсутствуют, и это защита, а не упущение:
        // перехват левой или правой отобрал бы у человека мышь целиком.
        Assert.False(HotkeyParser.TryParse("Mouse1", out _));
        Assert.False(HotkeyParser.TryParse("Mouse2", out _));
        Assert.False(HotkeyParser.IsMouseButton(0x01));
        Assert.False(HotkeyParser.IsMouseButton(0x02));
    }

    [Fact]
    public void КлавишиНеСчитаютсяКнопкамиМыши()
    {
